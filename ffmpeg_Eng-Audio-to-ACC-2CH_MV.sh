...
echo "Please refer to the aforementioned location for any artefacts older than 24 hours."
echo -e "\n"
...
echo -e "\n"
echo "Initiating copy of input file: $filename-ORIGINAL to $temp_directory -- this might take a few minutes depending on file size...."
...
for file in "${filtered_files[@]}"; do
  ...
  # Construct temporary input file path in the temp directory
  temp_input_file="${temp_directory}/${filename}.${extension}"
  ...
  echo -e "\n"
  echo "Input file: $filename-ORIGINAL has been copied to $temp_directory for processing..."
  echo -e "\n"
  echo "Running ffprobe command:"
  echo "ffprobe -show_streams -show_entries "format:stream" -v error -of json "$temp_input_file")"

  # Run ffprobe command to gather information about streams
  ffprobe_output=$(ffprobe -show_streams -show_entries "format:stream" -v error -of json "$temp_input_file")
  ...
  echo "Initiating ffmpeg: "
  echo -e "\n"
  echo "ffmpeg -hide_banner -i "$temp_input_file" -c:v copy -c:a aac -b:a 384k -map 0 -ac 2 "${map_audio_args[@]}" "$temp_output_file""
  echo "Please wait, as the shell will not update until after remux is completed...."

  # Run ffmpeg command to create the temporary output file
  ffmpeg_output=$(ffmpeg -hide_banner -i "$temp_input_file" -c:v copy -c:a aac -b:a 384k -map 0 -ac 2 "${map_audio_args[@]}" "$temp_output_file" 2>&1 | awk '!/frame=|_STATISTICS_WRITING_|_STATISTICS_TAGS|NUMBER_OF_FRAMES|NUMBER_OF_BYTES/ { print }')
